...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(_('Your changes are saved successfully'))
...
def messages():
    ...
    messages = current_user.messages_received.\
        order_by(Message.timestamp.desc()).paginate(page, current_app.config['POSTS_PER_PAGE'], False)
...
def search():
    ...
    next_url = url_for('main.search', q=g.search.q.data, page=page+1) \
        if total > page * current_app.config['POSTS_PER_PAGE'] else None
    prev_url = url_for('main.search', q=g.search.q.data, page=page - 1) \
        if page > 1 else None
